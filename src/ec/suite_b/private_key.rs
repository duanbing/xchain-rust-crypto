...
pub fn random_scalar(ops: &PrivateKeyOps) -> Result<Scalar> {
    let num_limbs = ops.common.num_limbs;
    let mut bytes = [0; ec::SCALAR_MAX_BYTES];
    let bytes = &mut bytes[..(num_limbs * LIMB_BYTES)];

    let mut rng = [0u8; ec::SCALAR_MAX_BYTES];
    rand::thread_rng().fill(&mut rng[..]);
...
pub fn generate_private_scalar_bytes(
    ...
    // DUANBING: 100 -> 1 and ensure the valid of candidate outside
    for _ in 0..1 {
...
pub fn check_scalar_big_endian_bytes(ops: &PrivateKeyOps, bytes: &[u8]) -> Result<()> {
...
pub fn scalar_from_big_endian_bytes(ops: &PrivateKeyOps, bytes: &[u8]) -> Result<Scalar> {
    // [NSA Suite B Implementer's Guide to ECDSA] Appendix A.1.2, and
    // [NSA Suite B Implementer's Guide to NIST SP 800-56A] Appendix B.2,
    // "Key Pair Generation by Testing Candidates".
    //
...
pub fn affine_from_jacobian(ops: &PrivateKeyOps, p: &Point) -> Result<(Elem<R>, Elem<R>)> {
